                                ...
                                <div style={{ zIndex: 10, backgroundColor: 'var(--bg-header)', boxShadow: 'var(--shadow-sm)', borderBottom: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
                                    ...
                                    <div style={{ padding: '0 16px 16px 16px' }}>
                                        <div style={{ position: 'relative', marginBottom: '16px', width: '100%' }}>
                                            ...
                                            <input type="text" placeholder="Search student to manage access..." value={blockSearch} onChange={(e) => setBlockSearch(e.target.value)} style={{ width: '100%', padding: '16px 16px 16px 48px', borderRadius: '16px', border: darkMode ? '1px solid rgba(255,255,255,0.4)' : 'none', backgroundColor: 'var(--bg-input)', fontSize: '16px', color: 'var(--text-main)', boxSizing: 'border-box' }} />
                                        </div>
                                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                            <button onClick={() => setSelectedAccessGrade(prev => prev === 'All' ? null : 'All')} style={{ padding: '10px 16px', borderRadius: '12px', border: 'none', backgroundColor: selectedAccessGrade === 'All' ? (darkMode ? '#f8fafc' : 'var(--text-main)') : (darkMode ? '#1e293b' : 'var(--bg-input)'), color: selectedAccessGrade === 'All' ? (darkMode ? '#0f172a' : 'white') : (darkMode ? '#f8fafc' : 'var(--text-main)'), fontWeight: '800', fontSize: '14px', cursor: 'pointer', transition: 'all 0.2s ease', minWidth: '48px' }}>
                                                All
                                            </button>
                                            {GRADES.map(g => (
                                                <button key={g} onClick={() => setSelectedAccessGrade(prev => prev === g ? null : g)} style={{ padding: '10px 16px', borderRadius: '12px', border: 'none', backgroundColor: selectedAccessGrade === g ? (darkMode ? '#f8fafc' : 'var(--text-main)') : (darkMode ? '#1e293b' : 'var(--bg-input)'), color: selectedAccessGrade === g ? (darkMode ? '#0f172a' : 'white') : (darkMode ? '#f8fafc' : 'var(--text-main)'), fontWeight: '800', fontSize: '14px', cursor: 'pointer', transition: 'all 0.2s ease', minWidth: '48px' }}>
                                                    {g}
                                                </button>
                                            ))}
                                        </div>
